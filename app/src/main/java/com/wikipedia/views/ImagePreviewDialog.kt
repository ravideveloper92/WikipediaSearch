...
import com.wikipedia.util.UriUtil.handleExternalLink
import com.wikipedia.util.UriUtil.resolveProtocolRelativeUrl
import com.wikipedia.util.log.L

class ImagePreviewDialog : com.wikipedia.page.ExtendedBottomSheetDialogFragment(), DialogInterface.OnDismissListener {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        suggestedEditsSummary = com.wikipedia.json.GsonUnmarshaller.unmarshal(SuggestedEditsSummary::class.java, arguments!!.getString(ARG_SUMMARY))
    ...
    override fun onStart() {
        ...
        BottomSheetBehavior.from(view!!.parent as View).peekHeight = com.wikipedia.util.DimenUtil
                .roundedDpToPx(com.wikipedia.util.DimenUtil.getDimension(R.dimen.imagePreviewSheetPeekHeight))
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        titleText!!.text = com.wikipedia.util.StringUtil.removeHTMLTags(com.wikipedia.util.StringUtil.removeNamespace(suggestedEditsSummary.displayTitle!!))
    ...
    private fun loadImageInfoIfNeeded() {
        if (suggestedEditsSummary.metadata == null) {
            disposables.add(com.wikipedia.dataclient.ServiceFactory.get(com.wikipedia.dataclient.WikiSite.forLanguageCode(suggestedEditsSummary.lang)).getImageExtMetadata(suggestedEditsSummary.title)
                    ...
                    .subscribe({ response ->
                        ...
                        com.wikipedia.util.log.L.e(caught)
                        showError(caught)
                    }))
        } else {
            setImageDetails()
    ...
    private fun setImageDetails() {
        if ((action == Action.ADD_CAPTION)
            ...
            addDetailPortion(getString(R.string.suggested_edits_image_preview_dialog_description_in_language_title,
                    com.wikipedia.WikipediaApp.getInstance().language().getAppLanguageLocalizedName(suggestedEditsSummary.lang)),
    ...
    private fun addDetailPortion(titleString: String, detail: String?, externalLink: String?) {
        if (!detail.isNullOrEmpty()) {
            ...
            view.detailTextView.text = com.wikipedia.util.StringUtil.strip(com.wikipedia.util.StringUtil.fromHtml(detail))
            if (!externalLink.isNullOrEmpty()) {
                view.detailTextView.setTextColor(com.wikipedia.util.ResourceUtil.getThemedColor(context!!, R.attr.colorAccent))
                ...
                view.detailsContainer.setOnClickListener {
                    dismiss()
                    com.wikipedia.util.UriUtil.visitInExternalBrowser(context, Uri.parse(externalLink))
                }
            } else {
    ...
    private val movementMethod = com.wikipedia.page.LinkMovementMethodExt { url: String ->
    ...
    private fun loadImage(url: String?) {
        ...
        com.wikipedia.util.log.L.v("Loading image from url: $url")
        com.wikipedia.views.ViewUtil.loadImageUrlInto(galleryImage, url)
    }
    ...
    companion object {
        ...
        fun newInstance(suggestedEditsSummary: SuggestedEditsSummary, action: Action): ImagePreviewDialog {
            ...
            val args = Bundle()
            args.putString(ARG_SUMMARY, com.wikipedia.json.GsonMarshaller.marshal(suggestedEditsSummary))
