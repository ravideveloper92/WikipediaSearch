...
internal class SuggestedEditsDisabledStatesView constructor(context: Context, attrs: AttributeSet? = null) : LinearLayout(context, attrs) {
    ...
    init {
        ...
        linkContainer1.setOnClickListener { com.wikipedia.util.UriUtil.visitInExternalBrowser(context, Uri.parse(context.getString(linkContainer1.tag as Int))) }
    }

    fun setPaused(message: String) {
        setDefaultState()
        messageTextView.text = com.wikipedia.util.StringUtil.fromHtml(message)
    ...
    fun setIPBlocked() {
        ...
        messageTextView.text = com.wikipedia.util.StringUtil.fromHtml(context.getString(R.string.suggested_edits_ip_blocked_message))
