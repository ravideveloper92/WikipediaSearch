...
import com.wikipedia.views.ViewAnimations;
...
import static com.wikipedia.Constants.InvokeSource.GALLERY_ACTIVITY;
import static com.wikipedia.Constants.InvokeSource.LINK_PREVIEW_MENU;
import static com.wikipedia.Constants.PREFERRED_GALLERY_IMAGE_SIZE;
...
import static com.wikipedia.util.StringUtil.addUnderscores;
import static com.wikipedia.util.StringUtil.removeUnderscores;
import static com.wikipedia.util.StringUtil.strip;
import static com.wikipedia.util.UriUtil.handleExternalLink;
import static com.wikipedia.util.UriUtil.resolveProtocolRelativeUrl;
...
public class GalleryActivity extends BaseActivity implements LinkPreviewDialog.Callback,
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            if (initialFilename != null) {
                funnel.logGalleryOpen(pageTitle, StringUtil.removeUnderscores(initialFilename));
            }
        } else {
    ...
    public void onShare(@NonNull GalleryItemFragment item, @Nullable Bitmap bitmap, @NonNull String subject, @NonNull PageTitle title) {
        ...
        if (bitmap != null && item.getMediaInfo() != null) {
            ShareUtil.shareImage(this, bitmap, new File(ImageUrlUtil.getUrlForPreferredSize(item.getMediaInfo().getThumbUrl(), Constants.PREFERRED_GALLERY_IMAGE_SIZE)).getName(),
    ...
    public void onActivityResult(int requestCode, int resultCode, final Intent data) {
        ...
        if (requestCode == ACTIVITY_REQUEST_DESCRIPTION_EDIT && resultCode == RESULT_OK) {
            FeedbackUtil.showMessage(this, data.getSerializableExtra(Constants.INTENT_EXTRA_ACTION) == DescriptionEditActivity.Action.ADD_CAPTION
    ...
    @OnClick(R.id.gallery_caption_edit_button) void onEditClick(View v) {
        ...
        startActivityForResult(DescriptionEditActivity.newIntent(this, title, null, summary, null, DescriptionEditActivity.Action.ADD_CAPTION, Constants.InvokeSource.GALLERY_ACTIVITY),
    ...
    @OnClick(R.id.gallery_caption_translate_button) void onTranslateClick(View v) {
        ...
        startActivityForResult(DescriptionEditActivity.newIntent(this, targetTitle, null, sourceSummary, targetSummary,
                (sourceSummary.getLang().equals(targetSummary.getLang())) ? DescriptionEditActivity.Action.ADD_CAPTION : DescriptionEditActivity.Action.TRANSLATE_CAPTION, Constants.InvokeSource.GALLERY_ACTIVITY),
                ACTIVITY_REQUEST_DESCRIPTION_EDIT);
    }
    ...
    @OnLongClick(R.id.gallery_license_container) boolean onLongClick(View v) {
        ...
        if (!TextUtils.isEmpty(licenseUrl)) {
            UriUtil.handleExternalLink(GalleryActivity.this, Uri.parse(UriUtil.resolveProtocolRelativeUrl(licenseUrl)));
        }
        return true;
    }
    ...
    private LinkMovementMethodExt linkMovementMethod =
        ...
        url = UriUtil.resolveProtocolRelativeUrl(url);
        if (url.startsWith("/wiki/")) {
            ...
            if (authority != null && WikiSite.supportedAuthority(authority)
                ...
                UriUtil.handleExternalLink(GalleryActivity.this, Uri.parse(url));
            }
        }
    });
    ...
    private void applyGalleryList(@NonNull List<MediaListItem> list) {
        ...
        if (initialFilename != null) {
            for (MediaListItem item : list) {
                ...
                String titleFromPage = StringUtil.removeNamespace(StringUtil.addUnderscores(initialFilename));
    ...
    public void updateGalleryDescription() {
        ...
        if (descriptionStr != null && descriptionStr.length() > 0) {
            ...
            descriptionText.setText(StringUtil.strip(descriptionStr));
        } else {
