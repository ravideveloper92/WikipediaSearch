...
import static com.wikipedia.Constants.API_QUERY_MAX_TITLES;
...
public final class BatchUtil {
    ...
    public static <T> void makeBatches(@NonNull final List<PageTitle> titles,
        ...
        for (int i = 0; i < titles.size(); i += Math.min(Constants.API_QUERY_MAX_TITLES, titles.size() - i)) {
            handler.handleBatch(titles.subList(i, i + Math.min(Constants.API_QUERY_MAX_TITLES, titles.size() - i)),
