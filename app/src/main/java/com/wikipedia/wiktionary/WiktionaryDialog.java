package com.wikipedia.wiktionary;
...
import com.wikipedia.analytics.WiktionaryDialogFunnel;
import com.wikipedia.dataclient.ServiceFactory;
import com.wikipedia.dataclient.WikiSite;
import com.wikipedia.dataclient.restbase.RbDefinition;
import com.wikipedia.page.ExtendedBottomSheetDialogFragment;
import com.wikipedia.page.LinkMovementMethodExt;
import com.wikipedia.page.PageTitle;
import com.wikipedia.util.StringUtil;
import com.wikipedia.util.log.L;
import com.wikipedia.views.AppTextView;
...
import static com.wikipedia.util.StringUtil.hasSectionAnchor;
import static com.wikipedia.util.StringUtil.removeSectionAnchor;
...
public class WiktionaryDialog extends ExtendedBottomSheetDialogFragment {
    ...
    private void loadDefinitions() {
        ...
        disposables.add(ServiceFactory.getRest(new WikiSite(pageTitle.getWikiSite().subdomain() + WIKTIONARY_DOMAIN)).getDefinition(StringUtil.addUnderscores(selectedText))
    ...
    private String sanitizeForDialogTitle(String text) {
        if (StringUtil.hasSectionAnchor(text)) {
            text = StringUtil.removeSectionAnchor(text);
        }
        return StringUtil.removeUnderscores(text);
    }
