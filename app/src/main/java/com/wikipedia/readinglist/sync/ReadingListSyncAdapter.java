...
import com.wikipedia.events.ReadingListsEnableDialogEvent;
import com.wikipedia.events.ReadingListsEnabledStatusEvent;
import com.wikipedia.events.ReadingListsMergeLocalDialogEvent;
import com.wikipedia.events.ReadingListsNoLongerSyncedEvent;
import com.wikipedia.page.PageTitle;
import com.wikipedia.readinglist.database.ReadingList;
import com.wikipedia.readinglist.database.ReadingListDbHelper;
import com.wikipedia.readinglist.database.ReadingListPage;
...
import static com.wikipedia.readinglist.sync.SyncedReadingLists.RemoteReadingList;
import static com.wikipedia.readinglist.sync.SyncedReadingLists.RemoteReadingListEntry;
...
public class ReadingListSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    public void onPerformSync(Account account, Bundle extras, String authority,
        ...
        try {
            ...
            List<SyncedReadingLists.RemoteReadingList> remoteListsModified = Collections.emptyList();
            List<SyncedReadingLists.RemoteReadingListEntry> remoteEntriesModified = Collections.emptyList();
            ...
            for (SyncedReadingLists.RemoteReadingListEntry remoteEntry : remoteEntriesModified) {
                // find the list to which this entry belongs...
                ReadingList eigenLocalList = null;
                SyncedReadingLists.RemoteReadingList eigenRemoteList = null;
                ...
                for (SyncedReadingLists.RemoteReadingList remoteList : remoteListsModified) {
            ...
            for (SyncedReadingLists.RemoteReadingList remoteList : remoteListsModified) {
                ...
                if (syncEverything) {
                    ...
                    List<SyncedReadingLists.RemoteReadingListEntry> remoteEntries = client.getListEntries(remoteList.id());
                    for (SyncedReadingLists.RemoteReadingListEntry remoteEntry : remoteEntries) {
            ...
            for (ReadingList localList : allLocalLists) {
                ...
                SyncedReadingLists.RemoteReadingList remoteList =
                        new SyncedReadingLists.RemoteReadingList(localList.title(), localList.description());
            ...
            for (ReadingList localList : allLocalLists) {
                ...
                List<SyncedReadingLists.RemoteReadingListEntry> newEntries = new ArrayList<>();
    ...
    private void createOrUpdatePage(@NonNull ReadingList listForPage,
                                    @NonNull SyncedReadingLists.RemoteReadingListEntry remotePage) {
    ...
    private PageTitle pageTitleFromRemoteEntry(@NonNull SyncedReadingLists.RemoteReadingListEntry remoteEntry) {
        WikiSite wiki = new WikiSite(remoteEntry.project());
        return new PageTitle(remoteEntry.title(), wiki);
    }

    private SyncedReadingLists.RemoteReadingListEntry remoteEntryFromLocalPage(@NonNull ReadingListPage localPage) {
        PageTitle title = ReadingListPage.toPageTitle(localPage);
        return new SyncedReadingLists.RemoteReadingListEntry(title.getWikiSite().scheme() + "://" + title.getWikiSite().desktopAuthority(), title.getPrefixedText());
    }
}
