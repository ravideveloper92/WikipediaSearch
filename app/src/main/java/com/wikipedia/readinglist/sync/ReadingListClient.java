...
import static com.wikipedia.readinglist.sync.SyncedReadingLists.RemoteReadingListEntryBatch;
...
public class ReadingListClient {
    ...
    public List<SyncedReadingLists.RemoteReadingList> getAllLists() throws Throwable {
        List<SyncedReadingLists.RemoteReadingList> totalLists = new ArrayList<>();
        int totalCycles = 0;
    ...
    public SyncedReadingLists getChangesSince(@NonNull String date) throws Throwable {
        List<SyncedReadingLists.RemoteReadingList> totalLists = new ArrayList<>();
        List<SyncedReadingLists.RemoteReadingListEntry> totalEntries = new ArrayList<>();
        int totalCycles = 0;
        String continueStr = null;
        do {
    ...
    public List<Long> addPagesToList(@NonNull String csrfToken, long listId, @NonNull List<SyncedReadingLists.RemoteReadingListEntry> entries) throws Throwable {
        ...
        List<SyncedReadingLists.RemoteReadingListEntry> currentBatch = new ArrayList<>();
        while (true) {
            currentBatch.clear();
            ...
            try {
                Response<SyncedReadingLists.RemoteIdResponseBatch> response
                        = ServiceFactory.getRest(wiki).addEntriesToReadingList(listId, csrfToken, new SyncedReadingLists.RemoteReadingListEntryBatch(currentBatch)).execute();
